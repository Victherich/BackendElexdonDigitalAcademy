...
router.post('/signup', async (req, res) => {
  ...
  try {
    ...
    const newUser = new User({ fullName, email, phoneNumber, password: hashedPassword, referrerCode });
    ...
    res.status(201).send({
      ...
      referrerCode: savedUser.referrerCode // Return the referrer code in the response
    });
  } catch (error) {
  ...
  try {
    ...
    res.send({
      ...
      user: {
        ...
        referrerCode: user.referrerCode // Include referrer code in login response
      }
    });
  } catch (error) {
